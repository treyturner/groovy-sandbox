...
/**
 * Scan a music and remove lesser quality redundant copies.
...
class RemoveRedundantMusic {
    ...
    static void main(String[] args) {
        ...
        def (minutes, secs) = calculateScanTime(scanStart, scanEnd)
...
//                    log.debug "Processing $processedCount: $dir.canonicalPath"
        ...
        superiorFormats.each { superiorFormat ->
            if (!betterCopyFound) {
                ...
                if (currentFormat == "flac") {
                    ...
                    def peerDirList = getPeerDirectories(directory)
                ...
                if (betterCopyPath) {
                    ...
                    if (betterCopy.exists()) {
                        ...
                        def success = desktop.moveToTrash(directory)
                        if (!success) { throw new Exception("Couldn't send to recycle bin") }
                    }
                }
            }
        }
    ...
    static List getPeerDirectories(File directory) {
        directory.parentFile.listFiles(new FilenameFilter() {
            ...
            boolean accept(File current, String name) {
                return new File(current, name).directory
            }
        })
    }
    ...
    static List<Long> calculateScanTime(LocalDateTime start, LocalDateTime end) {
        Duration scanDuration = Duration.between(start, end)
        long seconds = scanDuration.seconds
